...
def reflect_db():
    ...
    try:
        with engine.connect() as conn:
            ...
            with lock:
                # 2. Create a fresh automap_base to avoid stale mapper state.
                ...
                # 3. Reflect the public schema
                NewBase.prepare(autoload_with=engine)

                # 4. Also reflect the DLT dataset schema so tables like
                ...
                # 5. Atomically swap the module-level Base
                Base = NewBase
                _rebuild_class_map()

                # 6. Reset repository caches that hold model classes from the old Base
