from supabase import create_client
from core.config import storage_settings
import logging

logging.basicConfig(
    level=logging.DEBUG,
    format="%(asctime)s - %(module)s:%(lineno)d -%(levelname)s - %(message)s"
)

_client = None

def get_supabase_client():
    global _client
    if _client is None:
        logging.info("Initializing new Supabase client")
        if not storage_settings.STORAGE_URL or not storage_settings.STORAGE_KEY:
            logging.error("Storage credentials are not set")
            raise RuntimeError("Storage credentials are not set.")
        _client = create_client(storage_settings.STORAGE_URL, storage_settings.STORAGE_KEY)
        logging.info(f"Supabase client created successfully for URL: {storage_settings.STORAGE_URL}")
    else:
        logging.debug("Returning existing Supabase client")
    return _client