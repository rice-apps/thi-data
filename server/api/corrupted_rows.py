...
def _validate_and_cast(column_name: str, value: Any, column_type) -> Any:
    ...
    """
    logging.debug(f"Validating column '{column_name}' with value '{value}' for type {column_type}")
    if value is None:
        return None
    ...
    if isinstance(column_type, (Integer, BigInteger, SmallInteger)):
        try:
            ...
            logging.warning(f"Type validation failed for column '{column_name}': expected integer, got '{value}'")
    ...
    if isinstance(column_type, (Float, Numeric)):
        try:
            ...
            logging.warning(f"Type validation failed for column '{column_name}': expected number, got '{value}'")
    ...
    if isinstance(column_type, Boolean):
        ...
        if str(value).lower() in ('false', '0', 'no'):
            return False
        logging.warning(f"Type validation failed for column '{column_name}': expected boolean, got '{value}'")
    ...
    if isinstance(column_type, DateTime):
        ...
        try:
            ...
            logging.warning(f"Type validation failed for column '{column_name}': expected datetime (ISO format), got '{value}'")
    ...
    if isinstance(column_type, Date):
        try:
            ...
            logging.warning(f"Type validation failed for column '{column_name}': expected date (ISO format), got '{value}'")
...
def get_corrupted_rows(
    ...
    logging.info(f"Fetching corrupted rows with skip={skip}, limit={limit}")
    try: 
        items, total = crud.get_all_items(db, model_class, skip=skip, limit=limit)
        logging.info(f"Successfully retrieved {len(items)} corrupted rows (total: {total})")
        return {
        ...
        logging.error(f"Error getting corrupted rows: {str(e)}", exc_info=True)
...
def delete_corrupted_row(
    ...
    logging.info(f"Attempting to delete corrupted row entry with id: {row_id}")
    try:
        success = crud.delete_item(db, model_class, row_id)
        if not success:
            logging.warning(f"Corrupted row not found: {row_id}")
            raise HTTPException(status_code=404, detail="Corrupted row not found")
        logging.info(f"Corrupted row deleted successfully: {row_id}")
        return {"message": "Corrupted row deleted successfully"}
    except HTTPException:
        raise
    except Exception as e:
        logging.error(f"Error deleting corrupted row {row_id}: {str(e)}", exc_info=True)
...
def resolve_corrupted_row(
    ...
    """
    logging.info(f"Starting resolution for corrupted row - table: {target_table}, row_id: {row_id}")
    logging.debug(f"Resolution corrections: {resolution.corrections}")
    ...
    if not record:
        logging.warning(f"Record not found - table: {target_table}, row_id: {row_id}")
        raise HTTPException(status_code=404, detail=f"Record {row_id} not found in {target_table}")
    
    logging.debug(f"Record found in {target_table} for row_id: {row_id}")
    ...
    logging.debug(f"Retrieved column types for validation: {list(column_types.keys())}")

    try:
        for column, value in resolution.corrections.items():
            logging.debug(f"Processing correction for column '{column}' with value '{value}'")
            if not hasattr(record, column):
                logging.warning(f"Column '{column}' does not exist in table {target_table}")
            ...
            logging.debug(f"Successfully set column '{column}' to value '{value}'")
        
        logging.info(f"All corrections applied successfully for row_id: {row_id}")
    except ValueError as e:
        logging.error(f"Validation error during resolution: {str(e)}")
        raise HTTPException(status_code=400, detail=f"Validation Error: {str(e)}")
    except Exception as e:
        logging.error(f"Unexpected error during correction application: {str(e)}", exc_info=True)
        raise HTTPException(status_code=400, detail=f"Validation Error: {str(e)}")
    ...
    logging.info(f"Found {len(corrupted_entries)} corrupted entries to remove for row_id: {row_id}")

    try:
        
        for entry in corrupted_entries:
            logging.debug(f"Deleting corrupted entry with id: {entry.id}")
            db.delete(entry)
            
        db.commit()
        db.refresh(record)
        logging.info(f"Transaction committed successfully - row healed and {len(corrupted_entries)} error log(s) cleared")
        
    except Exception as e:
        db.rollback()
        logging.error(f"Database transaction failed during resolution: {str(e)}", exc_info=True)
    ...
    logging.info(f"Corrupted row resolved successfully - table: {target_table}, row_id: {row_id}")
    return result
