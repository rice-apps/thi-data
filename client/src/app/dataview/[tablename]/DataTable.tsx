...
import type { TableRow, CellError, PaginationParams, PaginatedResponse } from '@/types';
...
export default function DataTable({
  ...
  useEffect(() => {
    const handler = setTimeout(() => setDebouncedSearch(search), UI.DEBOUNCE_MS);
    ...
    const fetchFirstPage = async () => {
      ...
      try {
        ...
        const response = debouncedSearch
          ? await dataService.searchTableData(tablename, selectedColumn, debouncedSearch, params)
  ...
  }, [page, loading, hasMore, debouncedSearch, selectedColumn, tablename, dataService, showError]);
  ...
  const handleAddSubmit = async () => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        ...
        payload[key] = !isNaN(Number(trimmed)) && trimmed === Number(trimmed).toString()
          ? Number(trimmed)
          : trimmed;
      });
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      delete payload.id;
      await dataService.updateRow(tablename, modalState.row.id as string, payload);
      await refreshData();
      closeModal();
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  {columns.map((header) => (
                    <th key={header} className="px-6 py-4 text-left text-sm font-semibold text-slate-600 whitespace-nowrap">
                      {header}
                    </th>
                  ))}
                  <th className="px-6 py-4 text-right text-sm font-semibold text-slate-600 w-24">Actions</th>
                </tr>
              </thead>
                    ...
                    return (
                      <tr
                        ...
                        className={`transition-colors group ${
                          isCorrupted ? 'bg-red-50 border-l-4 border-red-400 hover:bg-red-100' : 'hover:bg-slate-50'
                        }`}
                      >
                        {columns.map((col) => {
                          const cellError = row._error_context?.[col];
                          ...
                          return (
                            <td
                              ...
                              className={`px-6 py-4 text-sm whitespace-nowrap ${
                                isCellCorrupted ? 'text-red-600' : row[col] != null ? 'text-slate-700' : 'text-slate-400 italic'
                              }`}
                            >
                              {renderCell(row, col)}
                            </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <ActionButton icon="edit" onClick={() => handleEdit(row)} title="Edit" />
                            <ActionButton icon="delete" onClick={() => handleDelete(row)} title="Delete" variant="danger" />
                          </div>
                        </td>
                      </tr>
                    );
          ...
          <div ref={observerTarget} className="p-6 text-center">
            ...
            {!hasMore && data.length > 0 && <span className="text-slate-400 text-sm">End of results</span>}
          </div>
        </div>
      </main>
      ...
      {(modalState.type === 'add' || modalState.type === 'edit') && (
        ...
        >
              ...
              <div key={col}>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 text-slate-700 bg-slate-50 border rounded-xl focus:outline-none focus:ring-2 focus:ring-[${COLORS.PRIMARY}]/30 focus:border-[${COLORS.PRIMARY}] focus:bg-white transition-all duration-200 ${
                    String(formData[col] ?? '').trim() === '' ? 'border-red-300' : 'border-slate-200'
                  }`}
          ...
          <ModalFooter>
            ...
            <Button onClick={modalState.type === 'add' ? handleAddSubmit : handleEditSubmit} loading={submitting}>
              Save
            </Button>
          </ModalFooter>
        </Modal>
      ...
      <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row md:items-center gap-4 justify-between">
        <div className="flex items-center gap-4 min-w-fit">
          ...
          <div>
            ...
            <h1 className="text-xl font-semibold text-slate-800 capitalize">{tablename}</h1>
          </div>
        </div>
...
function CorruptedCell({ cellError, onClick }: { cellError: CellError; onClick: () => void }) {
  return (
...
function ResolveModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} disableClose={loading} className="max-w-md">
      <div className="px-6 py-4 border-b border-slate-200 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold text-slate-800">Fix Corrupted Value</h2>
      ...
      <ModalBody className="space-y-4">
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 space-y-2">
          ...
          <p className="text-sm text-red-600">{cellError?.error ?? 'Validation failed'}</p>
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1.5">Corrected value</label>
          <input
            value={value}
