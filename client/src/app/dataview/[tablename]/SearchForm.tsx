...
export default function SearchForm({ tablename, headers, initialColumn, initialSearch }: SearchFormProps) {
  const router = useRouter();
  const [selectedColumn, setSelectedColumn] = useState(initialColumn || headers[0] || "");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.push(`/dataview/${tablename}?column=${selectedColumn}&search=${encodeURIComponent(searchText)}`);
  };
