...
class ApiService {
  ...
  static async getTablesWithMetadata(): Promise<{ tables: TableMetadata[] }> {
    return this.request<{ tables: TableMetadata[] }>('tables_with_metadata', { cache: 'no-store' });
  }

  static async getTableSchema(tableName: string): Promise<TableSchemaResponse> {
    return this.request<TableSchemaResponse>(`schema/${tableName}`, { cache: 'no-store' });
  }
  ...
  static async getTableData(
    ...
    return this.request<PaginatedResponse<TableRow>>(endpoint, { cache: 'no-store' });
  }
  ...
  static async createRow(tableName: string, data: Omit<TableRow, 'id'>): Promise<TableRow> {
  ...
  static async updateRow(tableName: string, id: string | number, data: Partial<TableRow>): Promise<TableRow> {
  ...
  static async deleteRow(tableName: string, id: string | number): Promise<void> {
